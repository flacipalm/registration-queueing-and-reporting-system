...
Hashset::Hashset(int size)
{
    ...
    reprarray = new Person*[maxsize];  // allocate space for the array of pointers
    // the for loop initialises all table entries to be undefined
    for (int i = 0; i < maxsize; i++)
    {
        reprarray[i] = 0;
    }
    Person *pt0 = new Person;  // This defines the fixed placeholder pointer
    pt_nil = pt0;
}
...
void Hashset::add(Person* person)
{
    hash<int> hashfunction; // use the predefined hashfunction to get "key" values
    int index;
    index = hashfunction(person->identification_number) % maxsize; // First determine the position index in the hash table, where the new value is stored, if free.
    ...
    {
        if (reprarray[index] != pt_nil && reprarray[index] == person)
...
void Hashset::remove(int id)
{
    ...
    {
        if (reprarray[index] != pt_nil && reprarray[index]->identification_number == id)
                    // item found
        {
    ...
    cout << "People whose id is " << id << " is not in the hashtable.\n";
    return;
}

Person* Hashset::member(int id)
{
    hash<int> hashfunction;  // use the "key" function for the type T (if defined)
    int index;
    index = hashfunction(id) % maxsize;
...
void Hashset::rehash(int newsize)
{
    if (newsize < 20)
        newsize = 20;
    int newnum = 0;
    Person **newarray = new Person*[newsize];   // allocate space for a new hashtable of the given new size
    ...
    for (int i = 0; i < maxsize; i++)  // we need to copy all existing entries to the new hash table
    {
        ...
        {
            Person* person = reprarray[i];
            hash<int> hashfunction;
            int index = hashfunction(person->identification_number) % newsize;
...
void Hashset::display(void) // only for test purposes
{
    ...
    for (int i = 0; i < maxsize; ++i)
    {
        if (reprarray[i] == 0)
            ...
            cout << "The entry " << i + 1 << " is " << reprarray[i]->name << " .\n";
    }
    return;
}
